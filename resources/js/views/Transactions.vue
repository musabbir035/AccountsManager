<template>
  <div>
      ...
      <div v-else>
        <div v-if="customer" class="mb-3">
          ...
          <a
            ...
            @click="$emit('open-ledger-modal', customer.id)"
            class="d-block"
          >
            Generate ledger for {{ customer.name }}
          </a>
          <router-link class="d-block" :to="{ name: 'Transactions' }">
    ...
    <b-modal
      ...
      no-close-on-backdrop
    >
      <item-details :detailsData="transactionData" />
    </b-modal>
...
import TransactionAddEditForm from "../components/TransactionAddEditForm.vue";
